...
def save_POV(Pdi, index, atoms, output_dir):
    ...
    pov_args = {
        ...
        'rotation': '45x, 0y, -180z', #45,0, -180 #Position of camera. If you want different angles, the format is 'ax, by, cz' where a, b, and c are angles in degrees
...
def predict_binding_Es_fast(atoms, Pd_interest = [], ind_index = None, view_flag =  False, output_descriptor = False, top_only = False, stable_info = False):
    ...
    if output_descriptor:
        ...
        return sitetype_list, GCNs, CN1s, CN2s, ratio_surface, Z_pos
    else: 
...
# class CO_interactions():
#     '''
#     CO-CO interactions object
#     Calculate the interaction energy
#     '''
#     def __init__(self, CO_pos, sitetype_list, CO_config, interactions_list = None):
...
#         G = lf.graphs_CO(occ, NN1 = False, unit_length = self.unit_length)
#         # Create mother graph for all sites
#         Gm  = G.gmothers(CO_pos, sitetype_list)
#         # Creat subgraph for only occupied site 
#         Gs = G.gconfigurations(CO_config)
...
#         '''
#         Input the CO graph, interaction dictionary list and interaction list in eV
#         return 
#         - the total interaction energy 
#         - interaction count list
#         '''
#         tol_length = 0.2 # the tolerance for bond length
#         edges = list(self.Gs.edges)
#         interactions_count = np.zeros(len(self.interactions))

#         for edge_i in edges: 
#             for i, interaction in enumerate(self.interactions):
#                 cond1 = (list(self.Gs.edges[edge_i]['edge_type']) in interaction['edge_type']) 
#                 if cond1: # Check if the type match
#                     cond2 = (np.abs(self.Gs.edges[edge_i]['length'] - interaction['length']) <= tol_length )
#                     if cond2: # Chekc if the length match
...
#         - interaction count list
#         '''
#         interactions_count = self.count_interactions() # count the interaction for this configuration
#         interactions_ev = np.array([inter_i['E'] for inter_i in self.interactions]) # extract the interaction energy
#         # The total amount of interaction per configuration 
#         total_interactions = np.dot(interactions_count, interactions_ev)  # eV
    
#         return total_interactions   
    
#     def cal_binding_Es_total(self, binding_Es):
#         '''
#         Calculate the total binding E by taking account interaction energies
#         '''
        
#         total_interactions = self.cal_interactions()
#         total_binding_Es = np.sum(binding_Es[self.CO_config])
#         return total_binding_Es + total_interactions
...
class CO_interactions():
    ...
    Calculate the interaction energy
    '''
    def __init__(self, CO_pos, sitetype_list, CO_config = None, interactions_list = None):
        ...
        # Initialize graph object
        self.G = lf.graphs_CO(occ, NN1 = True, unit_length = self.unit_length)
        # Create mother graph for all sites
        self.Gm  = self.G.gmothers(CO_pos, sitetype_list)
        
        # Creat subgraph for only occupied site 
        if not CO_config is None: 
        
            Gs = self.G.gconfigurations(CO_config)
                
            self.Gs = Gs
            self.CO_config = CO_config

        
    def define_config(self, CO_config):
        # Creat subgraph for only occupied site 
        Gs = self.G.gconfigurations(CO_config)
...
#%% Future directions
'''
Should remove the COs if Pd sites are gone
Use GA or basin hopping? to fastforward Pd structure change, right now it's too slow
'''


#%%
'''
Testing for a Pd20 structure
'''
if __name__ == "__main__":
#    # Given a CONTCAR file name
#    filename = 'pd20-no-CO-CONTCAR'
#    atoms = read(filename)
#
